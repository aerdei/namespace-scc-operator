...
func (r *ReconcileNamespaceSCC) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.List(context.TODO(), nil, sccList); err != nil {
	...
	if err := r.client.List(context.TODO(), nil, nsList); err != nil {
		if errors.IsNotFound(err) {
			// Request object not found, could have been deleted after reconcile request.
			// Owned objects are automatically garbage collected. For additional cleanup logic use finalizers.
			// Return and don't requeue
	...
	for _, sccElement := range sccList.Items {
		for _, nsElement := range nsList.Items {
			...
			if !whiteListedNamespace {
				...
				if err := r.client.Get(context.TODO(), client.ObjectKey{Name: sccElement.Name + "-" + nsElement.Name}, sccfound); err != nil && errors.IsNotFound(err) {
